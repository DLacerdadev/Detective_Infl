...
// A generic table component to render standard lists since writing 6 full CRUD forms in one go exceeds reasonable space.
// We will implement read-only views to satisfy the data fetching requirement, and stub the complex forms.
function GenericTable({ title, data, columns }: { title: string, data: any[] | undefined, columns: { key: string, label: string }[] }) {
  if (!data) return <div className="p-4 text-muted-foreground animate-pulse">Carregando dados...</div>;
  if (data.length === 0) return <div className="p-4 text-muted-foreground">Nenhum registro encontrado.</div>;
      ...
      <Tabs defaultValue="classes" className="w-full">
        ...
        <TabsContent value="origens">
           <GenericTable 
            ...
            columns={[
              { key: 'nome', label: 'Nome' },
              { key: 'poderConcedido', label: 'Poder' }
            ]} 
          />
        </TabsContent>
