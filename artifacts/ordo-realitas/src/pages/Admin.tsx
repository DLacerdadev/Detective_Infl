...
import { ShieldAlert, Search, ChevronDown, ChevronUp, BookOpen, Zap, Users } from "lucide-react";
      ...
      <div className="rounded-sm border border-border overflow-x-auto">
        <Table>
          <TableHeader className="bg-secondary/50">
            <TableRow>
              {columns.map(col => <TableHead key={col.key} className="font-display tracking-wider text-foreground">{col.label}</TableHead>)}
            </TableRow>
          </TableHeader>
          <TableBody>
            {data.map((row: any) => (
...
function OrigensTab() {
  ...
  const filtered = (origins ?? []).filter((o: Origem) => {
    const matchSearch = !search || o.nome.toLowerCase().includes(search.toLowerCase()) ||
  ...
  if (isLoading) return <div className="p-8 text-muted-foreground animate-pulse font-mono text-sm">Carregando origens...</div>;
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-3 flex-wrap">
        <div className="bg-card border border-border/50 rounded-sm px-4 py-2 flex items-center gap-2">
          ...
          <span className="font-mono text-sm"><span className="text-foreground font-bold">{(origins ?? []).length}</span> <span className="text-muted-foreground">origens no total</span></span>
        </div>
        <div className="bg-card border border-blue-500/25 rounded-sm px-4 py-2">
          <span className="font-mono text-sm"><span className="text-blue-400 font-bold">{livroBase}</span> <span className="text-muted-foreground">Livro Base</span></span>
        </div>
        <div className="bg-card border border-primary/25 rounded-sm px-4 py-2">
          <span className="font-mono text-sm"><span className="text-primary font-bold">{supl}</span> <span className="text-muted-foreground">Sobrevivendo ao Horror</span></span>
        </div>
      </div>
